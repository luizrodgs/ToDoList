...
SQL_TAREFA_POR_ID = (
    "SELECT ID, TITULO, DESCRICAO, DATA_FINAL from tbTarefa where ID = %s"
)
SQL_USUARIO_POR_ID = "SELECT ID, NOME, SENHA from tbUsuario where id = %s"
SQL_ATUALIZA_TAREFA = (
    "UPDATE tbTarefa SET TITULO=%s, DESCRICAO=%s, DATA_FINAL=%s where ID = %s"
)
SQL_BUSCA_TAREFA = "SELECT ID, TITULO, DESCRICAO, DATA_FINAL from tbTarefa"
SQL_CRIA_TAREFA = (
    "INSERT into tbTarefa (TITULO, DESCRICAO, DATA_FINAL) values (%s, %s, %s)"
)
...
class TarefaDao:
    ...
    def salvar(self, tarefa):
        ...
        if tarefa.id:
            cursor.execute(
                SQL_ATUALIZA_TAREFA,
                (tarefa.titulo, tarefa.descricao, tarefa.data, tarefa.id),
            )
        else:
            cursor.execute(
                SQL_CRIA_TAREFA, (tarefa.titulo, tarefa.descricao, tarefa.data)
            )
